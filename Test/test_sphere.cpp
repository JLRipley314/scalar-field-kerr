...
TEST(sphere_test, filter_is_TVD) {
   ...
   for (size_t it=0; it<Sphere::nlat(); it++) {
      po1[Sphere::indx_Sph(it,ip)] = 
         ...
         ;
      po2[Sphere::indx_Sph(it,ip)] = po1[Sphere::indx_Sph(it,ip)];
   }
   }
   Sphere::filter(po2);
   /* 
   ...
   for (size_t it=1; it<Sphere::nlat()-1; it++) {
      tv1 += 
         abs(po1[Sphere::indx_Sph(it  ,ip)]
         -   po1[Sphere::indx_Sph(it+1,ip)])
         +  
         abs(po1[Sphere::indx_Sph(it,ip  )]
          -  po1[Sphere::indx_Sph(it,ip+1)]);

      tv2 += 
         abs(po2[Sphere::indx_Sph(it  ,ip)]
         -   po2[Sphere::indx_Sph(it+1,ip)])
         +  
         abs(po2[Sphere::indx_Sph(it,ip  )]
          -  po2[Sphere::indx_Sph(it,ip+1)]);
   }
   }
